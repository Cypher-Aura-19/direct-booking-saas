...
- **Every new SQL function gets `revoke execute … from public, anon` (Supabase's default ACL grants anon by name, so revoking from public alone leaves anon able to execute) followed by an explicit `grant … to authenticated`.** `security definer` is not used in this milestone at all; if a task seems to need it, stop and ask.
